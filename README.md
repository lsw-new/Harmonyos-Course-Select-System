...
#### 演示登录账号（来自 `npm run seed`）

第 4 步执行 `npm run seed` 后，数据库会内置以下演示账号，可直接用于登录 Web 管理控制台或 App：

| 角色 | 账号（工号/学号） | 密码 | 用途 |
|---|---|---|---|
| 管理员 | `A20251001` | `Admin@2024` | 登录 `http://<服务器IP>:8090/admin/` Web 管理控制台 |
| 学生 | `2023307020941` | `Elysia@2024` | App 端学生登录 |

> ⚠️ 这两个是公开演示账号，**正式上线前务必改密码或删除**（在 Web 控制台「数据库管理」改 `accounts` 表，或重跑 `seed.js` 前改掉里面的密码常量）。
...
│   │   ├── index.html                           # 控制台骨架：登录视图 + 栏目（概览/学生/教师/选课/成绩/审批/反馈/评教/通知/验证码监控/审计/数据库）
...
│   │       ├── admin-teachers.routes.js         # 管理端教师管理：教师账号+资料增删改查 / 重置密码（teachers.manage 权限，T 编号自动生成）
│   │       ├── teacher.routes.js                # 教师端：工作台 / 我的课程 / 录入成绩 / 评教含按题目细分 / 课程通知含撤回 / 成绩申诉 / 资料含头像
...
> 教师端为本期新增的第三类角色（账号 role=`teacher`，由 `teacher_profiles` 落库，按教师姓名与课程/成绩/评教任务关联）。各功能均做**三端闭环**（学生 ↔ 教师 ↔ 管理端），数据走 Track B 后端。采用**底部导航栏结构**（工作台 / 课表 / 通知 / 评教 / 我的 五个 tab，复用学生/管理端同款 `TabBar`），视觉为「**教务工作台 · faculty console**」专业观感（身份头 + 读数条 + 状态色编码 + 分节眼眉），列表页均支持**下拉刷新**。

- 教师认证：工号密码登录，登录下发本人 `teacher_profiles`（姓名 / 学院 / 职称 / 邮箱 / 头像）
- 工作台（首页 tab）：身份头（姓名 + 职称·学院 + 评教均分读数）、授课 / 学生 / 待录入 / 评教四项指标读数条、待处理申诉徽标、授课课程工作队列（左侧条按成绩状态着色）
...
- **覆盖域**：认证（登录 / 注册 / 找回密码 / 邮箱验证码）、课程、选课（事务校验）、成绩、通知、消息中心、请假、反馈、评教、实践、成绩申诉、考试、对象存储、推送，**教师端**（工作台 / 我的课程 / 录入成绩 / 评教含按题目细分 / 课程通知含撤回 / 成绩申诉 / 资料含头像），以及管理端的学生管理、**教师管理**（教师账号+资料增删改查 / 重置密码，`teachers.manage` 细粒度权限）、成绩审核、审批、通知发布、角色权限、评教模板、审计日志、报表导出。
- **数据库迁移**：`server/migrations/`（001~022 权威建表脚本 + 班级名册 / 课表 / 成绩两段式 / 系统配置 / 消息 / 上传 / 推送 / 教师资料与头像 / 教师管理权限等）+ `npm run migrate` 幂等 runner。
...
> **完整测试文档（整合版）**：[`docs/TEST_DOCUMENTATION.md`](docs/TEST_DOCUMENTATION.md) —— 27 套件逐套件说明、36 项线上冒烟清单、客户端测试、历史测试记录时间线与复跑指引的单一入口。

后端 API 配套 **Jest + supertest** 自动化测试套件（`server/test/`）：

- **473 个用例 / 42 个套件**，覆盖登录与鉴权中间件、越权（IDOR）防护、管理端细粒度权限、限流、选课事务（轮次 / 容量 / 学分上限 / 时间冲突 + `FOR UPDATE` 行锁 + 失败回滚）、选课轮次状态机、成绩打分闭环、实践报名、注册与找回密码闭环、数据库管理防注入、验证码监控、**教师端**（工作台 / 名单归属鉴权 / 录入成绩 / 评教均值与按题目细分 / 课程通知与撤回 / 成绩申诉 / 头像）、**管理端教师管理**（增删改查 / 重置密码 / 权限守卫）、**可观测性**（健康检查丰富字段 / 结构化日志），以及各读写端点与全域错误分支（4xx 校验矩阵 / 事务回滚 / catch-500）与选课规则纯函数。
